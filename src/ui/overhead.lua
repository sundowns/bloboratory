Overhead = Class {
    ...
    draw = function(self)
        ...
        love.graphics.print('Round: '.. roundController.roundIndex .. ' / ' .. roundController.totalRounds, love.graphics.getWidth()*0.37, love.graphics.getHeight()*0.02)
        love.graphics.print('Lives: '.. playerController.livesRemaining .. ' / ' .. constants.MISC.STARTING_LIVES, love.graphics.getWidth()*0.49, love.graphics.getHeight()*0.02)
    end;
    ...
    display = function(self, windowWidth, windowHeight)
        ...
        if nk.windowBegin('Overhead', constants.UI.OVERHEAD.X*windowWidth, constants.UI.OVERHEAD.Y*windowHeight, constants.UI.OVERHEAD.WIDTH*windowWidth, constants.UI.OVERHEAD.HEIGHT*windowHeight) then
            ...
            nk.layoutRow('dynamic', (constants.UI.OVERHEAD.LAYOUTROW_HEIGHT*windowHeight), {0.1, 0.05, 0.05, 0.01, 0.05, 0.01, 0.05, 0.7})
            ...
            if playerController:currentDilationIs("FASTER") then
                nk.stylePush(self.styles.ACTIVE)
            end
            if nk.button("3x") then 
                playerController:setTimeDilation("FASTER")
            end 
            if playerController:currentDilationIs("FASTER") then
                nk.stylePop()
            end
